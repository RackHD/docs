...
Compute type nodes can be re-discovered/refreshed either by running a immediate refresh discovery graph or a delayed refresh discovery graph using the same nodeID from the original discovery process. The node catalog(s) will updated with new entries.
